...
void OLED_ClearRow(u8 start,u8 rowNum)
{
	uint8_t page,x;	    
	for(page=start; (page<page+rowNum)&&(page<PAGE); page++)  
	{  
		OLED_WR_Byte(0xb0 + page,OLED_CMD);    			//设置页地址（0~7）
		OLED_WR_Byte(0x00,OLED_CMD);      					//设置显示位置—列低地址
		OLED_WR_Byte(0x10,OLED_CMD);      					//设置显示位置—列高地址   
...
void OLED_FloatNumber(uint8_t x,uint8_t y, double num)
{
	...
	OLED_ShowString(x,y,(char *)".");
	x += gap;
	OLED_ShowNum(x,y,(int32_t)((num-integerPart)*10),1);				//原参数100000,5小数，修改为显示10,1位小数
}
...
/****************************************************
函数名：OLED_ShowValue(double Value)
参数：测量的厚度值Value
功能：将测量的厚度显示到OLED
返回值：无
****************************************************/
void OLED_ShowValue(double Value)
{
	if(OLED == 1)
	{
		if(Value < 100)
		{
			Value = (fabs(Value) + 0.05)*10/10;
			OLED_ClearRow(4,2);
			OLED_FloatNumber(8*4,4,Value);			
		}
		else
		{
			Value = Value + 0.5;
			...
			{
				OLED_ClearRow(4,2);
				OLED_ShowNum(8*4,4,(uint32_t)Value,3);
			}
			else
			{
				OLED_ClearRow(4,2);
				OLED_ShowNum(8*4,4,(uint32_t)Value,4);
			}			
		}		
	}
}
